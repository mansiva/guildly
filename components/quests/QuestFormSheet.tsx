...
export function questFormToFirestore(form: QuestFormData): {
  ...
  deadline: Timestamp | null; xpReward: number; repeat: boolean;
} {
  ...
  if (form.duration === 'custom') {
    ...
    deadline = deadlineForDuration(form.duration, form.weekOffset);
  }
...
export default function QuestFormSheet({ editing, onClose, onSave }: Props) {
  ...
  try {
    const deadline = form.duration === 'custom'
      ...
      : deadlineForDuration(form.duration, form.weekOffset);
      ...
      <div className="relative w-full max-w-[480px] bg-white rounded-t-3xl px-5 pt-5 pb-20 overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            {(form.duration === 'weekly' || form.duration === 'monthly') && (
              <div className="flex gap-2 mt-2">
                {(['this', 'next'] as const).map(o => {
                  const label = form.duration === 'weekly'
                    ? (o === 'this' ? '📅 This Friday' : '📅 Next Friday')
                    : (o === 'this' ? '🗓 This month' : '🗓 Next month');
                  return (
                    <button key={o} onClick={() => setForm(f => ({ ...f, weekOffset: o }))}
                      className={`flex-1 py-2.5 px-3 rounded-xl text-sm font-medium border transition-all ${
                        form.weekOffset === o ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-gray-50 text-gray-700 border-gray-200'
                      }`}>
                      {label}
                    </button>
                  );
          ...
          {/* Repeat toggle — only for weekly/monthly */}
          {(form.duration === 'weekly' || form.duration === 'monthly') && (
            <button
              type="button"
              onClick={() => setForm(f => ({ ...f, repeat: !f.repeat }))}
              className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl border transition-all ${
                form.repeat ? 'bg-indigo-50 border-indigo-300' : 'bg-gray-50 border-gray-200'
              }`}
            >
              <div className="flex items-center gap-2">
                <span className="text-base">🔁</span>
                <div className="text-left">
                  <p className={`text-sm font-semibold ${form.repeat ? 'text-indigo-700' : 'text-gray-700'}`}>Repeat each {form.duration === 'weekly' ? 'week' : 'month'}</p>
                  <p className="text-xs text-gray-400">Auto-create next cycle when this one ends</p>
                </div>
              </div>
              <div className={`w-10 h-6 rounded-full transition-colors relative ${
                form.repeat ? 'bg-indigo-600' : 'bg-gray-300'
              }`}>
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all ${
                  form.repeat ? 'left-5' : 'left-1'
                }`} />
              </div>
            </button>
          )}
